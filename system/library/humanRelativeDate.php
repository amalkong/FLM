...
class HumanRelativeDate{
	...
	protected function calcTimeDiffString(){
		...
		if($diff > 0){
			...
			$diff = abs($diff);
			if($diff < $this->magic_5_mins) $this->string = 'just now';
			else if ($diff < $this->magic_15_mins) $this->string = 'a few minutes ago';
			else if ($diff < $this->magic_30_mins) $this->string = 'in the last half hour';
			else if ($diff < $this->magic_1_hour) $this->string = 'in the last hour';
			else  if ($diff < ($this->magic_1_hour * 2)) $this->string = '1 hour ago';
			else $this->string = floor($diff / $this->magic_1_hour) . ' hours ago'; //$this->string = 'today at ' . date('H:i' , $this->event_timestamp);
		}
	}
