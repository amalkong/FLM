<?php defined('PBD_FLM') or exit('Hacking Attempt detected .... Go Fuck Yourself, ACCESS DENIED!');
    if (($roleID < 4))  {
       echo'<div class="box"><div class="message error"><p>Access Denied!,You are not authorised to access this section, your user role does not permit you to edit fixtures.</p></div></div>';
	...
	$file_type = (isset($_GET['filetype'])) ? $_GET['filetype'] : 'fixtures';
	if($file_type == 'fixtures') {
	...
	if($file_type == 'fixtures') {
		$fixture_filename = $match_date[0].'_fixture.txt';
		$NewFixture_FileName = $path.$fixture_filename ;
	} else if($file_type == 'results') {
		$result_filename = $match_date[0].'_result.txt';
		$NewResult_FileName = $path.$result_filename ;
	}
	$form_url = $_SERVER['PHP_SELF'].'?mode=admin&amp;section=manage&amp;filetype='.$file_type.'&amp;'.$extra_url_bit.'&amp;'.$stat_type.'='.$selected_statFiles.'&amp;c=ok';
